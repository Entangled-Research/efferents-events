...
def test_diagnostics_scrubs_provider_and_owner_tokens_and_reports_eval_sync(tmp_path, monkeypatch):
    ...
    folder.mkdir()
    (folder / "registration.json").write_text(json.dumps({"lab_id": "chem-lab", "display_name": "Chemistry lab", "owner_id": owner.owner_id}))
    ...
    assert report["labs"][0]["name"] == "Chemistry lab"
    assert report["labs"][0]["id"] == "chem-lab"
